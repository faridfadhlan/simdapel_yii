...
class MessagesController extends Controller
{
	...
	{
            $this->model_pesans[0] = Messages::model()->findByPk($id);
            $this->getChildren($this->model_pesans[0]->id);
            var_dump($this->model_pesans);exit;
		$this->render('view',array(
			'models'=>$this->model_pesans
		));
	}
	...
	public function actionIndex()
	{
		...
		$dataProvider=  Messages::model()->findAll($criteria);
	...
	protected function performAjaxValidation($model)
	{
        ...
        public function getChildren($parent, $level=1) { 
            $criteria = new CDbCriteria;
            $criteria->condition='parent_id=:id';
            $criteria->params=array(':id'=>$parent);
            $this->model_pesans[$level] = Messages::model()->findAll($criteria);
            foreach ($this->model_pesans[$level] as $data) {
                //echo str_repeat(' — ', $level) . $key->name . "<br />";
                $this->getChildren($data->id, $level+1);
            }
        }
}
